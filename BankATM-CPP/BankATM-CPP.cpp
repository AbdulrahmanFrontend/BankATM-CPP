...
enum enATMMainMenuOptions {
    ...
    eDeposit, eCheckBalance, eLogout
};
...
enum enQuickWithdrawOption {
    eTen = 1, eFifty, eOneHundred,
    eTwoHundreds, eFourHundreds, eSixHundreds,
    eEightHundreds, eOneThousand, eExit
};
...
void depositAccountToClientByAccountNumber(double depositedAmount, bool quick = 1) {
    if (quick && -1 * depositedAmount > currentClient.accountBalance) {
        performQuickWithdrawOption((enQuickWithdrawOption)short(readNum(
        "Amount exceeds the balance, you can't withdraw up to: "
...
void showQuickWithdrawScreen() {
    ...
    performQuickWithdrawOption((enQuickWithdrawOption)short(readNum("Choose what to withdraw [1 to 8]: ")));
}
void showNormalWithdrawScreen() {
    ...
    do {
        ...
        while (abs(withdrawAmount) > currentClient.accountBalance)
            withdrawAmount = readNum("Amount exceeds the balance, you can't withdraw up to: "
            + to_string(currentClient.accountBalance)
            + ".\nPlease, enter another value is less to withdraw: ");
...
void showATMMainMenu() {
    ...
    cout << "\t[5] Logout.\n";
    cout << "===========================================\n";
    performATMMainMenuOption((enATMMainMenuOptions)short(readNum("Choose what you want to do [1 to 5]: ")));
}
void showLoginScreen() {
